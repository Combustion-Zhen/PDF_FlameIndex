C***********************************************************************BEGIN-HDR
C
C  Copyright (c) 1997-2006 Reaction Design.  All rights reserved.
C
C  $Header: /KINetics_API/trunk/prebuild/chemkin460/user_routines/cklib_user_routines.f 20    10/23/09 11:52a Jkottalam $
C
C  $NoKeywords: $
C***********************************************************************END-HDR
C
      SUBROUTINE CKUPROG (LOUT, NII, I, ITYP, T, C, ICKWRK, RCKWRK,
     1                    RPROG, IFLAG)
!DEC$ IF DEFINED (DLL_EXPORT)
!DEC$ ATTRIBUTES DLLEXPORT :: CKUPROG
!DEC$ ENDIF
C
C  START PROLOGUE
C
C  SUBROUTINE CKUPROG  (LOUT, NII, I, ITYP, T, C, ICKWRK, RCKWRK,
C                       RPROG, IFLAG)
C  Template for user-modify of default reaction rates-of-progress.
C  CAUTIONS!!!
C  (1)
C  CKUPROG should only modify RPROG and possibly the error flag (IFLAG);
C  use local variables and arrays for any other work;
C  some local arrays have been provided as an example.
C  (2)
C  CKUPROG should NOT call any other CHEMKIN routine that returns
C  reaction rates-of-progress (CKQYP, CKQXP, CKQYR, CKKFKR),
C  to avoid any recursion.
C
C  INPUT
C  LOUT      - if positive, the unit number of an open output file
C  NII       - Integer scalar; the total number of reactions.
C  I         - Integer scalar; the current reaction index.
C  ITYP      - Integer scalar; an optional type parameter which may
C              have been provided in the mechanism, otherwise, zero.
C  T(*)      - Real array, temperature(s).
C                 cgs units, K
C  C(*)      - Real array, concentrations of the species;
C              the length of this array is KK, the total species count.
...
C  RPROG     - Real scalar, the default rate of progress of
C              reaction I;
C                 cgs units, moles/(cm**3*sec)
C
C  OUTPUT
C  RPROG     - User-provided reaction rate of progress for reaction I.
C              rate-of-progress of reaction I.
C  IFLAG     - Integer scalar, provides reverse communication to
C              the calling routine about the user-rate modify.
C  END PROLOGUE
      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
      INCLUDE 'user_routines_interface.inc'
      DIMENSION ICKWRK(*), RCKWRK(*), C(*), T(*)
C
C     Example: create local array space to facilitate calculations;
C     KMAX is an upper limit for species data,
C     IMAX is an upper limit for reaction data.
      PARAMETER (LIPAR=1000, LRPAR=1000, KMAX=100, IMAX=500)
      DIMENSION IPAR(LIPAR), RPAR(LRPAR), WT(KMAX)
C
C     Example: find additional mechanism parameters
      CALL CKINDX (ICKWRK, RCKWRK, MM, KK, II, NFIT)
C
C     Example: set error flag if inconsistent data
      IF (NII .NE. II) THEN
         IFLAG = 1
         RETURN
      ENDIF
C
C     Example: partition local work array to store molecular weights
      NWT = 1
      IF (KK .LE. LRPAR) CALL CKWT (ICKWRK, RCKWRK, RPAR(NWT))
C
C     Example: store molecular weights in local array
      IF (KK .LE. KMAX)  CALL CKWT (ICKWRK, RCKWRK, WT)
C
C     Local copy of Q(I)
      RTEMP = RPROG
C
C     Copy the default value for this reaction into RPROG
      RPROG = RTEMP
C
      RETURN
      END
C
      SUBROUTINE CKUPROD (LOUT, NKK, KTYP, T, C, ICKWRK, RCKWRK, RPROD,
     1                    IFLAG)
!DEC$ IF DEFINED (DLL_EXPORT)
!DEC$ ATTRIBUTES DLLEXPORT :: CKUPROD
!DEC$ ENDIF
C
C  START PROLOGUE
C
C  SUBROUTINE CKUPROD  (LOUT, NKK, KTYP, T, C, ICKWRK, RCKWRK, RPROD,
C                       IFLAG)
C  Template for user-modify of default species rates-of-production.
C  CAUTIONS!!!
C  (1)
C  CKUPROD should only modify RPROD and possibly the error flag (IFLAG);
C  use local variables and arrays for any other work;
C  some local arrays have been provided as an example.
C  (2)
C  CKUPROD should NOT call any other CHEMKIN routine that returns
C  species rates-of-production
C  (CKCDXP, CKCDXR, CKCDYP, CKCDYR, CKCTC, CKCTXP, CKCTXR, CKCTYP, CKCTYR,
C   CKCDC, CKWC, CKWXR, CKWYR, CKWXP, CKWYP, CKDHXP, CKDHYP),
C  to avoid any recursion.
C
C  INPUT
C  LOUT      - if positive, the unit number of an open output file
C  NKK       - Integer scalar; the mechanism species count.
C  KTYP(*)   - Integer array; the [optional] rate-modify types which may
C              have been provided in the mechanism, or nominally one.
C  T(*)      - Real array, temperature.
C                 cgs units, K
C  C(*)      - Real array, concentrations of the species;
C              the length of this array is NKK, the total species count.
C                 cgs units, mole/cm**3
C  ICKWRK(*) - Integer workspace array; the length of the array is LENICK.
C  RCKWRK(*) - Real    workspace array; the length of the array is LENRCK.
C  RPROD(*)  - Real array, prodution rates of the species;
C              the length of the array is NKK, the total species count.
C                 cgs units, moles/(cm**3*sec)
C
C  OUTPUT
C  RPROD(*)  - The user-provided replacement values for the
C              rates-of-production of the species.
C  IFLAG     - Integer scalar, provides reverse communication to
C              the calling routine about user-modify.
C  END PROLOGUE
C
      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
      INCLUDE 'user_routines_interface.inc'
...
C
C
C     Example: obtain pressure from given concentrations
...
C----------------------------------------------------------------------C
C                                                                      C
      SUBROUTINE CKWYP_USER  (P, T, Y, ICKWRK, RCKWRK, WDOT)
      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
